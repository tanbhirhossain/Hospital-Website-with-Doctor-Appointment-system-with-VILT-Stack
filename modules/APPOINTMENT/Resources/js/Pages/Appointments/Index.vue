...
const cancelingAppointment = ref(null)
...
// Submit cancellation
const submitCancelAppointment = () => {
    if (!cancelingAppointment.value) return

    router.post(
        route('appointments.cancel', cancelingAppointment.value.id), // adjust route name if needed
...
<div class="grid grid-cols-2 gap-3 border-t border-slate-200 bg-white px-6 py-5">
    ...
    >
        {{ form.processing ? 'Saving...' : isEditing ? 'Update Appointment' : 'Save Appointment' }}
            ...
            <!-- Cancel modal -->
            <!-- Cancel modal -->
            <div v-if="cancelModalOpen" class="fixed inset-0 z-[80] flex items-center justify-center bg-slate-900/45 p-5">
                <div class="w-full max-w-[460px] rounded-[18px] bg-white p-[22px] shadow-[0_24px_80px_rgba(15,23,42,0.25)]">
                    <div class="mb-[18px] flex items-start justify-between gap-4">
                        <div>
                            <h3 class="text-xl font-bold text-slate-900">Cancel Appointment</h3>
                            <p class="mt-[5px] text-[13px] text-slate-500">Provide a reason (optional) and confirm cancellation.</p>
                        </div>
                        <button type="button" class="rounded-[10px] bg-slate-100 p-2 text-slate-500 transition hover:bg-slate-200" @click="closeCancelModal">
                            <X :size="18" />
                        </button>
                    </div>

                    <div v-if="cancelingAppointment" class="mb-4 flex flex-col gap-1 rounded-[14px] border border-slate-200 bg-slate-50 p-3.5">
                        <strong class="text-[15px] text-slate-900">{{ cancelingAppointment.patient_name }}</strong>
                        <span class="text-[13px] text-slate-500">{{ doctorName(cancelingAppointment) }}</span>
                        <span class="text-[13px] text-slate-500">
                            {{ formatDate(String(cancelingAppointment.appointment_date).substring(0, 10)) }} — {{ formatTime(cancelingAppointment.start_time) }}
                        </span>
                    </div>

                    <div class="mb-5">
                        <label class="mb-2 block text-[13px] font-extrabold text-slate-700">Cancellation Reason</label>
                        <textarea
                            v-model="cancelReason"
                            rows="3"
                            placeholder="Optional: Why is this appointment being cancelled?"
                            class="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 text-sm text-slate-900 transition focus:border-indigo-500 focus:bg-white focus:outline-none focus:ring-4 focus:ring-indigo-100"
                        ></textarea>
                    </div>
                    ...
                    <div class="flex justify-end gap-2.5">
                        ...
                        <button
                            type="button"
                            class="rounded-[10px] bg-rose-600 px-3.5 py-2.5 text-sm font-semibold text-white transition hover:bg-rose-700"
                            @click="submitCancelAppointment"
                        >
                            Yes, Cancel
                        </button>
                    </div>

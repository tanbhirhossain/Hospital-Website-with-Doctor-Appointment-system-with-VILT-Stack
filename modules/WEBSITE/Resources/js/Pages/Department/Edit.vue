...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Edit', href: `/departments/${props.department.id}/edit` },
];

const slugWasEdited = ref(true);
const showSeo = ref(false);
const showCustomCss = ref(!!props.department.custom_css);

const form = useForm<DepartmentForm>({
    _method: 'put',
    name: props.department.name,
    slug: props.department.slug,
    tagline: props.department.tagline ?? '',
    serial: props.department.serial,
    is_active: props.department.is_active,
    text_icon: props.department.text_icon ?? '',
    'bg-color': props.department['bg-color'] ?? '',
    'text-color': props.department['text-color'] ?? '',
    color: props.department.color ?? '',
    custom_css: props.department.custom_css ?? '',
    parent_id: props.department.parent_id,
    shortDesc: props.department.shortDesc ?? '',
    descriptions: props.department.descriptions ?? '',
    banner_image: null,
    image: null,
    icon_image: null,
    meta_title: props.department.meta_title ?? '',
    meta_description: props.department.meta_description ?? '',
    canonical_url: props.department.canonical_url ?? '',
    og_title: props.department.og_title ?? '',
    og_description: props.department.og_description ?? '',
    indexable: props.department.indexable,
});

const imagePreviews = ref<DepartmentMedia>({
    banner_image: props.department.media.banner_image,
    image: props.department.media.image,
    icon_image: props.department.media.icon_image,
});

const availableParents = computed(() =>
    props.parentOptions.filter((p) => p.id !== props.department.id),
);

const mediaCount = computed(() =>
    Object.values(props.department.media).filter(Boolean).length,
);
...
const submit = () => {
    form.post(route('admin.departments.update', props.department.id), {
        forceFormData: true,
        preserveScroll: true,
    });
};
...
const updateFile = (event: Event, field: 'banner_image' | 'image' | 'icon_image') => {
    ...
    form[field] = file;
    imagePreviews.value[field] = file ? URL.createObjectURL(file) : props.department.media[field];
};
            ...
            <div class="mx-auto max-w-5xl space-y-6">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <h1 class="mt-1 text-3xl font-extrabold tracking-tight text-gray-900">Edit Department</h1>
                        <p class="mt-1 text-base text-gray-500">Update the details for this department</p>
                    </div>
                ...
                <form class="space-y-6" @submit.prevent="submit">
                    ...
                    <section class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
                        <div class="flex items-center gap-3 border-b border-gray-100 px-6 py-4">
                            ...
                            <h2 class="text-sm font-bold text-gray-900">Department Info</h2>
                        </div>
                        <div class="grid grid-cols-2 divide-x divide-gray-100 lg:grid-cols-4">
                            <div class="flex items-center gap-3 px-5 py-3.5">
                                <Hash class="size-4 text-gray-400" />
                                <div>
                                    <p class="text-[11px] font-medium uppercase text-gray-400">ID</p>
                                    <p class="text-sm font-bold text-gray-900">#{{ department.id }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 px-5 py-3.5">
                                <Clock class="size-4 text-gray-400" />
                                <div>
                                    <p class="text-[11px] font-medium uppercase text-gray-400">Created</p>
                                    <p class="text-sm font-bold text-gray-900">{{ department.created_at ?? '—' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 px-5 py-3.5">
                                <Building2 class="size-4 text-gray-400" />
                                <div>
                                    <p class="text-[11px] font-medium uppercase text-gray-400">Parent</p>
                                    <p v-if="department.parent" class="text-sm font-bold text-gray-900">{{ department.parent.name }}</p>
                                    <p v-else class="text-sm text-gray-400">Root level</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 px-5 py-3.5">
                                <Image class="size-4 text-gray-400" />
                                <div>
                                    <p class="text-[11px] font-medium uppercase text-gray-400">Media</p>
                                    <p class="text-sm font-bold text-gray-900">{{ mediaCount }}/3</p>
                                </div>
                            </div>
                        </div>
                    </section>
                    ...
                    <section class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100">
                        <div class="flex items-center gap-3 border-b border-gray-100 px-6 py-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400">Update images and icons</p>
                            </div>
                        </div>
                                ...
                                >
                                    ...
                                    <div v-if="!imagePreviews.banner_image" class="flex flex-col items-center gap-2 py-4">
                                        ...
                                        <div class="text-center">
                                            ...
                                            <p class="text-[11px] text-gray-400">1200 × 400px recommended</p>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div v-if="showSeo" class="space-y-5 border-t border-gray-100 p-6">
                                <div class="grid gap-5 sm:grid-cols-2">
                                    <div class="space-y-1.5">
                                        ...
                                        <Input id="meta_title" v-model="form.meta_title" :disabled="form.processing" placeholder="SEO title" class="h-11 rounded-xl border-gray-200 bg-gray-50/50 focus:bg-white" />
                                ...
                                <div class="space-y-1.5">
                                    ...
                                    <textarea id="meta_description" v-model="form.meta_description" rows="2" :disabled="form.processing" placeholder="Description for search results..." class="w-full rounded-xl border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm text-gray-700 focus:border-gray-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-gray-900 disabled:opacity-50" />
